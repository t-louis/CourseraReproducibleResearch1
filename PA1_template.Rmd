...
The 5-minute interval **`r max_interval_time`** is where we have **`r max_total_steps`** maximum average number of steps.

## Imputing missing values

```{r echo=FALSE}
# Retrieve the total amount of NAs measurements
number_nas <- sum(is.na(processed_raw_data$steps))
percent_nas <- round(mean(is.na(processed_raw_data$steps))*100)
```
The current processed dataset has currently **`r number_nas`** missing values which is equivalent to **`r percent_nas`%** of the total dataset.
...
```
```{r echo=FALSE}
# It is expected that both original dataset ad zero fill dataset
# have exactly the same distribution with same mean and same median
plot_hist_daily_total_steps(
    processed_raw_data, 
    plot_title = "Distribution daily total steps (Original Data)",
    plot_color = "blue"
)
```
```{r echo=FALSE}
plot_hist_daily_total_steps(
    clean_zero_fill_data, 
    plot_title = "Distribution daily total steps (Zero Fill Method)",
    plot_color = "yellow"
)
```
...
```{r echo=FALSE}
# Original data showed that there is roughly an increase of 1209 steps
# (~13%) in the daily mean when applying mean imputation and an increase of 1360
# steps (~14.5%) on the daily mean when applying median imputation. The median 
# remains unchanged comparing to the original dataset in all 3 cases. .
plot_hist_daily_total_steps(
    clean_mean_fill_data, 
    plot_title = "Distribution daily total steps (Mean Method)",
    plot_color = "orange"
)
```
```{r echo=FALSE}
plot_hist_daily_total_steps(
    clean_median_fill_data, 
    plot_title = "Distribution daily total steps (Median Method)",
    plot_color = "pink"
)
```
...
```{r echo=FALSE}
plot_day_time_average_steps(
    clean_mean_fill_data[clean_mean_fill_data$day_type == "weekday",],
    plot_title = "Time average steps (Weekdays)",
    plot_color = "black"
)
```
```{r echo=FALSE}
plot_day_time_average_steps(
    clean_mean_fill_data[clean_mean_fill_data$day_type == "weekend",],
    plot_title = "Time average steps (Weekend)",
    plot_color = "blue"
)
```
